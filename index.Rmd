...
download.file("https://raw.githubusercontent.com/menawhalen/WiDS_Data_Dive/master/Redcross.rds", 
              "data/Redcross.rds",
              method="curl")
keep_zip_codes <- read_rds("data/Redcross.rds") %>% 
  as_tibble() %>% 
  clean_names() %>% 
...
il_map_dat <- readOGR(dsn="data/tl_2010_17_zcta510/")
il_map_dat@data$id <- rownames(il_map_dat@data)
il_points <- fortify(il_map_dat, region = "ZCTA5CE10") %>% 
  as_tibble() %>% 
...
zip_income_pop_dat <- read_csv(file = "data/population_income.csv") %>% 
  clean_names() %>% 
  mutate(zip = zip %>% as.character())
